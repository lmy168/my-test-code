...
figure(i*10);
% figure(i*90)= figure('Color',[1 1 1]);
[x,y,z]=sphere;
for index123 = 1:length(circleCenter(:,1))                                                      % i从1到5
    mesh(r(index123)*x+circleCenter(index123,1),r(index123)*y+circleCenter(index123,2),r(index123)*z+circleCenter(index123,3));    % 绘制网格图形 circleCenter(i,1)、circleCenter(i,2) 和 circleCenter(i,3) 是第 i 个圆柱体或球体的中心位置的 x、y 和 z 坐标。
    hold on;                                                                            
end
...
if rand < 0.5
   ...
   l_handle = plot3([T.v(count).xPrev; newPoint(1)], [T.v(count).yPrev; newPoint(2)],[T.v(count).zPrev; newPoint(3)], 'b', 'Linewidth', 2);  %在三维空间中绘制一条从父节点到最新节点的蓝色路径线段，并返回该路径线段的句柄
    ...
    for rewire_index = 1:length(nearIndexList)            %对nearlndexList列表中的每个索引进行循环迭代，该列表包含与新节点（newPoint）相邻的节点的索引
        if(nearIndexList(rewire_index) ~= temp_parent)    % 若不是之前计算的最小cost的节点
            ...
            if(newCost < T.v(nearIndexList(rewire_index)).totalCost)    % 检查新路径的总代价是否比相邻节点当前记录的总代价更小。是的话需要剪枝
                ...
                if ~checkPath3(x_neib, newPoint, circleCenter,r) % 检查从树中最接近新点的节点到新点的路径是否可行。如果不可行，则跳过当前迭代，继续下一个迭代
                    continue;
                end
    ...
    if(findPath == 1)                       %如果已经找到路径，则执行下面的代码块
        ...
        if(update_count == UpdateTime)      %将update_count等于Update Time,则表示需要更新路径
            ...
            for j = 2:length(path.pos)                     %通过for循环遍历路径中的每个节点，绘制相应的路径线段，将图形句柄存储在resHandleList中
                res_handle = plot3([path.pos(j).x; path.pos(j-1).x;], [path.pos(j).y; path.pos(j-1).y],[path.pos(j).z; path.pos(j-1).z;], 'g', 'Linewidth', 1);
